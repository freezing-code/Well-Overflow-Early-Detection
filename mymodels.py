...
class TransformerBatchNormEncoderLayer(nn.modules.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.3, activation="relu"):
        ...
        self.norm1 = nn.LayerNorm(d_model, eps=1e-5)  # normalizes each feature across batch samples and time steps
...
class TSTransformerdecoder(nn.Module):
    def __init__(self, feat_dim, d_model, dropout=0.3):
...
class AE(nn.Module):
    def __init__(self, feat_dim, max_len, d_model,):
        super(AE, self).__init__()

class TCN(nn.Module):
    def __init__(self, feat_dim, max_len, d_model, kernel_size=3, dropout=0.3, cnndepth=3, cnnhidden_channels=64,hidden_time_dim=256):
        super(TCN,self).__init__()
        self.project_inp=causal_cnn.CausalCNN(in_channels=feat_dim, channels=cnnhidden_channels, out_channels=d_model,
                                             depth=cnndepth, kernel_size=kernel_size,time_dim=max_len,hidden_time_dim=hidden_time_dim)
        self.act=nn.GELU()
        self.max_len = max_len
        self.d_model = d_model
        self.feat_dim = feat_dim
    ...
    def forward(self, X):
        """
        :param X: (batch_size, time_dim, feat_dim)
        :return: [seq_length, batch_size, d_model]
        """
        ...
        inp = self.act(inp).permute(1, 0, 2)
        return inp

class MT(nn.Module):
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, kernel_size=3, dropout=0.3,
                 pos_encoding='fixed', activation='gelu', cnndepth=3, cnnhidden_channels=64,hidden_time_dim=256,freeze=False):
...
class TCNST(nn.Module):
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, cnndepth, cnnhidden_channels,
                 ...
                 pos_encoding='fixed', activation='gelu',hidden_time_dim=256, freeze=False):
        super(TCNST, self).__init__()
        # original is MT
        self.TSTencoder = TCN(feat_dim, max_len, d_model, kernel_size,  dropout,
                            cnndepth, cnnhidden_channels,hidden_time_dim)
        self.TSTdecoder = TSTransformerdecoder(feat_dim, d_model)
...
class MTSTco(nn.Module):
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, cnndepth, cnnhidden_channels,
        ...
        self.TSTencoder = MT(feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, kernel_size, dropout,
                             pos_encoding, activation, cnndepth, cnnhidden_channels,hidden_time_dim, freeze)
        self.TSTdecoder = TSTransformerdecoder(feat_dim, d_model)
